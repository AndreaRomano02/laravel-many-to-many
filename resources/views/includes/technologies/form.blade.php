@if ($technology->exists)
    <form class="row" action="{{ route('admin.technologies.update', $technology) }}" method="POST"
        ...
        @method('PUT')
    @else
        <form class="row" action="{{ route('admin.technologies.store') }}" method="POST" enctype="multipart/form-data"
            novalidate>
@endif
...
<div class="col-6">
    ...
    <input type="text" class="form-control @error('label') is-invalid @enderror" id="label" name="label"
        value="{{ old('label', $technology->label) }}" placeholder="Inserisci un tipo...">
    @error('label')
...
<div class="col-2">
    ...
    <input type="color" name="color" class="form-control form-control-color @error('color') is-invalid @enderror"
        id="color" value="{{ old('color', $technology->color) }}" title="Choose your color">
    @error('color')
        <div class="invalid-feedback">
            {{ $message }}
        </div>
